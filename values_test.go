...
func TestGetValues(t *testing.T) {
	...
	// validate the flags
	require.Regexp(t, "^[0-9]{4}(-[0-9]{2}){2}T([0-9]{2}:){2}[0-9]{2}Z$", fl["BuildDate"])
	require.Regexp(t, "^[0-9a-f]{7}$", fl["GitCommit"])
	require.Equal(t, "master", fl["GitBranch"])
	require.Equal(t, "clean", fl["GitState"])
}
